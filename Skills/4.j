        ...
        if (n > 0) then
            ...
            loop
                ...
                if (GetUnitState(picked, UNIT_STATE_LIFE) > 0) then
                    ...
                    set bj_lastCreatedUnit = CreateUnit(Naruto_Player, 'h015', GetUnitX(picked), GetUnitY(picked), GetRandomReal(0, 360))
                    call SetUnitScale(bj_lastCreatedUnit, 1.20, 1.20, 1.20)
                    call UnitApplyTimedLife(bj_lastCreatedUnit, 1112820806, 3.00)
                endif
            endloop
            call DestroyGroup(g)
            set g = null
        ...
        if (n > 0) and (isMoving) then
            ...
            if (isMoving == false) then
                ...
                loop
                    ...
                    call SetUnitVertexColor(dummies[n], 255, 60, 0, 255)
                    call SetUnitTimeScale(dummies[n], GetRandomReal(1.5,  2.8))
                endloop

                call DestroyTreeXY(x, y, garea)
                call SoundPatch_Spell(udg_soundPatch_Naruto + "\\Naruto\\RasenshurikenEff.wav", x, y)
                set n = 70
